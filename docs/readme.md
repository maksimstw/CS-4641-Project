...
In the next few weeks, we will try to fine tune the hyper parameters, and did error analysis to further understand the reason why smaller model achieve better results. In addition, smaller models with better attention mechanism might be able to achieve the same performance as deeper models. Hence, the seek for better attention mechanism besides squeeze and excitation network was also worth exploring. Furthermore, an early stopping technique could also be implemented to prevent overfitting. Currently, all 6 models were trained for 15 epochs. However, if the f1 score decreased consecutively for 3 epochs, we might be able to assume that the model started to overfit, and thus we should terminate the training earlier. 
